---
audio: false
generated: true
image: false
lang: en
layout: post
model: x-ai/grok-4.1-fast
title: Registering for TOEFL via NEEA China
translated: false
type: note
---

Question: How to register for the TOEFL exam on https://toefl.neea.cn

Answer: 

**Note:** The website https://toefl.neea.cn is the official registration portal for the TOEFL iBT test in **Mainland China** (operated by the National Education Examinations Authority - NEEA, under the Ministry of Education). It is primarily in Chinese, though some parts may offer English options. This is different from the global ETS site used in most other countries.

Here are the general steps to register for the TOEFL exam on https://toefl.neea.cn (based on official procedures for Mainland China test-takers):

1. **Visit the website**  
   Go to https://toefl.neea.cn.

2. **Read important information first**  
   Before registering, carefully read the "考生须知" (Candidate Information/Notes) section. This covers exam rules, requirements, policies, test day procedures, and important notices. You must agree to the terms and the NEEA online registration agreement.

3. **Create or log in to your account**  
   - If you don't have an account, register a new NEEA candidate account (usually requires a valid Chinese Mainland phone number for verification, your name exactly as on your ID, date of birth, and other personal details).  
   - The name and date of birth must **exactly match** the identification document (usually a valid passport or combination of Chinese ID + other documents) you will use on test day.  
   - If you already have an account, log in.

4. **Complete your profile**  
   Fill in or verify your personal information accurately (this profile will hold your registration details and later your scores).

5. **Register for the test**  
   - Select "Register" or the equivalent button for TOEFL iBT.  
   - Choose your preferred test date, test center location (available in many cities across China), and time slot. Seats fill up quickly, so book early (popular dates can be taken months in advance).  
   - Check availability for your desired date and city.

6. **Pay the test fee**  
   The fee is typically around 2100–2200 CNY (subject to change; confirm on the site).  
   Payment is usually made through supported Chinese methods (e.g., Alipay, China Merchants Bank, or other online banking options). International credit cards may not always be accepted.

7. **Confirm registration and download/print confirmation**  
   After payment, you will receive a confirmation. Download and print the registration form or admission ticket if required. Bring this and your ID to the test center.

**Additional important tips:**
- Register as early as possible — seats are limited and popular test dates fill quickly.
- You can usually register up to about 7 days before the test date (late registration may incur extra fees if allowed).
- For the **TOEFL iBT Home Edition** in China, use a different site: https://toefl.cn/at-home.
- If you need special accommodations (e.g., disability-related), contact NEEA support early.
- For the latest dates, fees, test centers, and any policy changes, always check directly on https://toefl.neea.cn or the candidate notes section.
- If you encounter issues, use the online customer service chat on the site or contact NEEA support.

This process is specific to Mainland China. If you're registering from outside Mainland China (e.g., Singapore), use the global ETS site at https://www.ets.org/toefl instead.

References:  

- [https://toefl.neea.cn/](https://toefl.neea.cn/) – Official NEEA TOEFL registration site for Mainland China  
- [https://www.ets.org/toefl/test-takers/ibt/about/testing-options.html](https://www.ets.org/toefl/test-takers/ibt/about/testing-options.html) – ETS official guidance directing China test center registration to toefl.neea.cn  
- [https://www.ets.org/pdfs/toefl/toefl-ibt-bulletin.pdf](https://www.ets.org/pdfs/toefl/toefl-ibt-bulletin.pdf) – TOEFL iBT Information Bulletin (2025-2026) confirming China registration via NEEA