...
The Tiger Trade app (published by Tiger Brokers, also known as Tiger Trade: Invest Globally on Google Play or similar in regional app stores) allows trading of US stocks, options, ETFs, futures, and more. The latest version as of September 2025 is 9.4.0.2, which introduced enhancements to TigerAI for analysis and shareholder functions showing buying/selling trends [1].

To display buy/sell options in the app (assuming you're referring to stock or options trading pages):

1. **Open the App and Navigate to a Stock**:
   - Search for a stock (e.g., AAPL) on the home or markets tab.
   - Tap the stock to open its detail page, which includes chart, news, and analysis tabs.

2. **Show Buy/Sell Buttons for Stocks**:
   - On the stock page, scroll to the bottom or tap the "Trade" icon.
   - Buy and Sell buttons appear, allowing you to enter order details (e.g., quantity, order type like market or limit).
   - No persistent buy/sell overlays exist on the chart in the mobile app (unlike the desktop version, which has configurable chart trading settings with buttons [2] [2]).

3. **Show Buy/Sell for Options**:
   - On the stock page, tap the "Options" tab to view the options chain.
   - Scroll to select expiration dates. Each option shows bid/ask prices with implied buy/sell actions when tapped (buy to open for calls/puts, or sell if long a position [3] [4]).
