---
audio: false
generated: true
image: false
lang: en
layout: post
model: anthropic/claude-4.5-sonnet
title: Hong Kong Mainland Talent Scheme Requirements
translated: false
type: note
---

I'll search for the current requirements of Hong Kong's Admission Scheme for Mainland Talents and Professionals (ASMTP).Based on the search results, here are the **key requirements for the Admission Scheme for Mainland Talents and Professionals (ASMTP)**:

## Applicant Requirements

**Educational Background & Qualifications:**
- A bachelor's degree or higher, or relevant technical qualifications, professional expertise, or proven professional experience

**Employment Status:**
- Must have secured a job relevant to their academic qualifications or work experience that cannot be readily taken up by the local workforce
- The position must demonstrate genuine need and not be easily fillable by local Hong Kong residents

**Remuneration:**
- Salary and benefits package must be commensurate with the prevailing market rate for professionals in the relevant field in Hong Kong

**Personal Eligibility:**
- Must be a Chinese resident of Mainland China
- Must have no security objection and no known record of serious crime
- Good character with clean criminal record

## Employer Requirements

- Applications must be submitted by the employer in Hong Kong, and the employer must be a registered company in the HKSAR

## Streamlined Processing

Applications may receive faster processing if either:
- The position is on the Talent List of shortage occupations, or the annual salary package is HK$2 million or above (in these cases, employers are exempted from proving local recruitment difficulties)

## Stay Duration

- Successful applicants are normally granted an initial stay of 36 months, or according to the employment contract duration (whichever is shorter)
- After 7 years of continuous residence, applicants can apply for permanent residency
- Spouses and children under 18 can accompany the applicant

The scheme has **no quota limit** and **no sector restrictions**, making it accessible to qualified Mainland professionals across all industries.
