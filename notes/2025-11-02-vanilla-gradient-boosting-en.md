...
Here's a from-scratch implementation of a simple Gradient Boosting Regressor, inspired by Friedman's 2001 paper. It uses **decision stumps** (single-split trees, i.e., depth-1 trees) as weak learners to keep things vanilla and simple—no scikit-learn, just NumPy for numerics and Matplotlib for plotting. This captures the core: fitting to pseudo-residuals (negative gradients of squared loss), shrinkage (learning rate), and additive updates.

The code is self-contained and runnable in a Python environment with NumPy/Matplotlib.
...
- **GradientBoostingRegressor**:
  ...
  - Iteratively fits stumps to residuals \\( r = y - F_{m-1} \\) (negative gradient).
  - Updates \\( F_m = F_{m-1} + \nu \cdot h_m \\) with shrinkage \\( \nu = 0.1 \\) for regularization.
  - No stochastic subsampling or line search here for simplicity (paper suggests them for scale-up).
- **Performance**: On this toy data, expect low MSE (~0.01 on test). It converges quickly (plot shows decreasing loss).
- **Limitations**: Stumps limit complexity; for real use, extend to deeper trees. Handles regression; adapt for classification via log-loss gradients.
- **Run It**: Paste into a script/notebook. Outputs MSE and plots mirroring the paper's convergence/accuracy focus.

For extensions (e.g., deeper trees, classification), let me know!
