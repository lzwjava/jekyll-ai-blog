...
2. **Access Parking**: Tap "Park Services" (公园服务) > "Parking Reservation" (预约停车) > "Online Appointment" (在线预约).
3. **Select the Park and Lot**: Search for "Lixiang Park" (荔香公园), then choose "North Gate Parking Lot" (北门停车场). Check real-time availability—spots fill fast (the lot holds about 100-150 cars).
4. **Book Your Spot**: Bind your vehicle first if not done (enter license plate, owner details, and verify via SMS). Then input arrival time (slots are typically 1-4 hours), confirm, and get a QR code.
5. **At the Gate**: Drive up to the north gate (off Shennan Dadao, near Nanhai Ave), scan your QR at the barrier. No extra payment needed—parking is free for the reserved duration (up to 4 hours max per slot; overstays may incur fees).

- **Alternative Entry**: Or use this direct link in WeChat: [smartum.sz.gov.cn/tcyy/parking/lot-mobile/main](https://smartum.sz.gov.cn/tcyy/parking/lot-mobile/main). For a full guide, follow "Shenzhen Local Bao" (深圳本地宝) WeChat and message "公园停车" (park parking).
- **Tips**: Book via mobile data (Chinese SIM ideal for verification). If full, try the west gate (no reservation but smaller) or nearby street parking (¥2-5/hour). North gate is closest to the main lawns and lychee groves. Electric vehicle chargers available inside.
