...
- **Codex Updates**: Now generally available with Slack integration, a dedicated Codex SDK, and enterprise-grade controls for secure coding assistance.
- **GPT-5 Pro**: The most advanced model in the API lineup, optimized for precision tasks like complex reasoning and analysis.
- **gpt-realtime-mini**: A compact voice model that's 70% cheaper than its larger counterpart, ideal for real-time audio applications.
- **gpt-image-1-mini**: A cost-effective image generation model, 80% less expensive, for efficient visual content creation.
