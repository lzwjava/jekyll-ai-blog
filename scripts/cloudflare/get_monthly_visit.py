...
def get_rum_sites(headers, account_id):
    ...
    if response.status_code != 200:
        print(f'Error fetching RUM sites: {response.status_code} {response.text}', file=sys.stderr)
        return []
    data = response.json()
    if not data.get('success'):
        print(f'API error fetching RUM sites: {json.dumps(data, indent=2)}', file=sys.stderr)
        return []
    return data['result']

def main():
    parser = argparse.ArgumentParser(description='Get monthly page views & unique visitors from Cloudflare Web Analytics (RUM)')
    ...
    for site in sites:
        ...
        if response.status_code != 200:
            print(f'Error for site {site_name} ({site_tag}): {response.status_code} {response.text}', file=sys.stderr)
            continue
        ...
        if 'errors' in data:
            ...
            if 'errors' in data:
                print(f'GraphQL errors for site {site_name} ({site_tag}): {json.dumps(data["errors"], indent=2)}', file=sys.stderr)
                continue

            try:
                metrics = data['data']['viewer']['accounts'][0]['analyticsEngineDatasetBindings'][0]['analyticsEngineMetrics1dGroups']
                ...
                print(f'No data for site {site_name} ({site_tag}) via Analytics Engine', file=sys.stderr)
                continue
        else:
            try:
                ...
                site_page_views = sum(group['sum']['pageViews'] for group in daily_groups)
