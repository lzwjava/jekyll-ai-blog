...
source_file = os.path.expanduser("~/.bashrc")

parent_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), ".."))
# Create the path to the config directory within the parent directory
target_file = os.path.join(parent_dir, "config", ".bashrc")
...
content = re.sub(r'export SERVER_IP=".*"\n?', "", content)
content = re.sub(r'export CLASH_DOWNLOAD_URL=".*"\n?', "", content)
