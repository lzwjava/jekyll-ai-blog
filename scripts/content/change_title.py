        ...
        for file in files:
            if file.endswith(".md"):
                ...
                updated_content = re.sub(
                    rf"\[(.*?)\]\({re.escape(old_url_slug)}([^)]*?)\)",
                    f"[{new_title}]({old_url_slug}\2)",
                    updated_content,
                )
...
def change_post_title(file_path, new_title):
    ...
    if "title" in front_matter:
        ...
        with open(file_path, "w", encoding="utf-8") as f:
            f.write(updated_content)
        print(
            f'Updated title in {os.path.basename(file_path)} from "{old_title}" to "{new_title}"'
        )
    else:
...
def main():
    parser = argparse.ArgumentParser(
        description="Change the title of a Markdown post and update related files."
    )
