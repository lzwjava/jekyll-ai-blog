---
audio: false
generated: true
image: false
lang: zh
layout: post
title: 无咖啡因咖啡的隐藏化学风险
translated: true
type: note
---

**问题：** https://yinwang1.substack.com/p/8be 的 Substack 帖子说了什么？

**回答：**

这个 Substack 帖子由王垠撰写，用中文撰写，标题为 **低咖啡因咖啡豆是怎么来的**（英文翻译为“How Decaffeinated Coffee Beans Are Made” 或 “Where Low-Caffeine Coffee Beans Come From”）。

**发布日期：** 2024 年 2 月 5 日  
**作者：** 王垠（在他的通讯“垠的备忘录” / Yin's Memo 上）

文章表达了对脱咖啡因（decaf）咖啡的强烈担忧。作者解释说他避免摄入含咖啡因饮料，也拒绝喝脱咖啡因咖啡。他的原因是大多数脱咖啡因咖啡豆使用有机溶剂处理来去除咖啡因，他认为这会留下有问题的化学残留物。

他引用了 The Conversation 的一篇文章（https://theconversation.com/how-is-decaf-coffee-made-and-is-it-really-caffeine-free-215546），并描述了两种主要的脱咖啡因方法，特别是重点关注基于溶剂的方法。

帖子中的关键点：

- 有两种主要去除咖啡因的方法，但溶剂方法（使用有机溶剂）是最常见的，因为它更便宜。
- 溶剂方法有两种变体：**direct** 和 **indirect**。

**直接溶剂方法（详细描述）：**
- 咖啡豆先被蒸气处理。
- 然后浸泡在 dichloromethane（methylene chloride，二氯甲烷）或 ethyl acetate 中以溶解咖啡因。
- 浸泡后，豆子再次被蒸气处理以去除残留溶剂。

作者质疑第二次蒸气处理步骤是否真的能消除所有溶剂痕迹。他对此表示怀疑，认为：
- 这些是挥发性化合物，但它们已经长时间浸入豆子中。
- 不确定溶剂是否完全蒸发、是否与咖啡豆中的成分发生化学结合，或者是否被困在豆子结构内部。

帖子似乎是付费墙（标记为“Paid”），公开可见的部分大约在对直接方法的这种怀疑性评论和残留化学物质处结束。完整文章很可能继续讨论间接溶剂方法，可能还有 Swiss Water Process 或 CO₂ 方法（作为更安全的替代方案），并解释为什么作者完全避免脱咖啡因咖啡。

总体信息：作者对传统脱咖啡因咖啡高度怀疑，因为溶剂提取过程可能导致化学污染，并推荐避免饮用。

**参考资料：**

- [原始 Substack 帖子](https://yinwang1.substack.com/p/8be)
- [引用的来源文章 (The Conversation)](https://theconversation.com/how-is-decaf-coffee-made-and-is-it-really-caffeine-free-215546)