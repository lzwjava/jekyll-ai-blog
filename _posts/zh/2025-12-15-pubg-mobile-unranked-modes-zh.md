...
title: PUBG Mobile非排位模式列表
translated: true
type: note
---

**问题：PUBG 的非排名模式有多少种？它们是什么？**
...
- **狂怒战车模式（Rage Gear Mode）**：高机动性、强调激进装备的模式。
- **火力对决模式（Payload Mode）**：基于目标PVPVE的模式，有货物需要捕获/防御（在休闲游戏中非常受欢迎）。
- **团队死亡竞赛（Team Deathmatch，TDM）**：快节奏的 4v4 或小队死亡竞赛，地图包括 Warehouse、Hangar、Ruins（可在非排名竞技场选项卡中找到）。
- **感染模式（Infection Mode）**：僵尸生存模式，玩家会变成感染者。
- **战争模式（War Mode）**：占领/统治风格，有控制点（例如，Town 地图）。
- **地铁逃生（Metro Royale）**：Rogue-like的射击游戏，需要拾荒和逃离（独立的经济系统）。
