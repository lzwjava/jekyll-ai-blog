---
audio: false
generated: true
image: false
lang: zh
layout: post
title: Tritan 与 PC 瓶材对比
translated: true
type: note
---

**问题：Tritan 是什么，它与用于瓶子的 PC 材料相比如何？**

**回答：**

**Tritan** 是 Eastman Chemical Company 自 2007 年以来开发的一种共聚酯塑料的品牌名称。它是一种透明、耐用且抗碎裂的材料，专门设计为 polycarbonate (PC) 的更安全替代品，主要由于对 Bisphenol A (BPA) 的担忧。Tritan 由单体制成，包括 dimethyl terephthalate (DMT)、cyclohexanedimethanol (CHDM) 和 2,2,4,4-tetramethyl-1,3-cyclobutanediol (CBDO)，并且不含 **bisphenols**（如 BPA 或 BPS）或 phthalates。

它常用于可重复使用的水瓶、婴儿产品、食品容器和饮具，因为它具有玻璃般的透明度、韧性和抗冲击、抗污渍和抗异味的特性。

**Polycarbonate (PC)** 是一种坚固、透明的热塑性塑料，广泛用于瓶子（例如旧款 Nalgene 型号）。它以出色的抗冲击性和耐用性而闻名，但含有 BPA，这种化学物质会渗出，尤其是暴露在热量下，导致与内分泌干扰相关的健康担忧。

### Tritan 与 PC 用于瓶子的比较

| Feature              | Tritan                                      | Polycarbonate (PC)                          | Winner/Notes                              |
|----------------------|---------------------------------------------|---------------------------------------------|-------------------------------------------|
| **BPA Content**      | BPA-free (and free of BPS/other bisphenols) | Contains BPA                                | Tritan (safer for health concerns)        |
| **Safety**           | Generally considered safer; no bisphenol leaching | Potential BPA leaching, especially with heat | Tritan                                    |
| **Clarity**          | Excellent, glass-like, long-lasting         | Excellent initially, but can yellow/cloud over time | Similar, but Tritan maintains better     |
| **Impact Resistance/Durability** | Very high, shatter-resistant                | Very high, nearly unbreakable               | Similar or slight edge to PC in extreme impacts |
| **Heat Resistance**  | Up to ~109°C; good for hot liquids but not boiling | Up to ~120–130°C; better for higher heat    | PC (but heat increases BPA risk)          |
| **Chemical/Stain Resistance** | Excellent, resists stains/odors/acids       | Good, but can stain or retain odors over time | Tritan                                    |
| **Dishwasher Safe**  | Yes, holds up well over many cycles         | Yes, but may degrade clarity faster         | Tritan                                    |
| **Cost**             | Higher                                      | Lower                                       | PC                                        |
| **Common Use in Bottles** | Preferred for modern BPA-free bottles (e.g., Nalgene switched to Tritan) | Older bottles; less common now due to BPA concerns | Tritan (current trend)                    |

**用于瓶子的总体比较**：如今，Tritan 通常比 PC 更受可重复使用水瓶的青睐。主要优势是其 **BPA-free 成分**，解决了导致许多品牌（如 Nalgene）从 PC 转向 Tritan 的健康担忧。它提供了相当或更好的长期透明度、抗污渍性和安全性，而不会牺牲太多耐用性。然而，一些研究对受压 Tritan（如在 UV/热量下）的潜在雌激素活性提出了疑问，尽管制造商测试和许多来源确认其食品安全接触。

如果避免任何潜在化学渗出是首要考虑，Tritan 是大多数用户的更好选择。对于极端高温或最大韧性，PC 可能仍用于某些应用，但如今在饮具中较少见。

**参考资料：**

- [Tritan copolyester - Wikipedia](https://en.wikipedia.org/wiki/Tritan_copolyester)
- [Frequently Asked Questions | Tritan and Tritan Renew - Eastman](https://www.eastman.com/en/products/brands/tritan/about/faqs)
- [Tritan vs. PC: Which BPA-Free Plastic is Truly Best for Your Reusable Water Bottles?](https://ckmold.com/tritan-vs-pc-which-bpa-free-plastic-is-truly-best-for-your-reusable-water-bottles)
- [Tritan Safety - Eastman](https://www.eastman.com/en/products/brands/tritan/about/safety)
- [Are the BPA-Free Plastics Like Tritan Safe? - NutritionFacts.org](https://nutritionfacts.org/blog/are-the-bpa-free-plastics-like-tritan-safe)
